...
function readJsonFile(filePath, fallback = {}) {
  try {
    ...
    err(`Error parsing JSON from ${filePath}: ${e.message}`);
    if (e instanceof SyntaxError) {
      try {
        const backupPath = `${filePath}.corrupt-${Date.now()}`;
        fs.renameSync(filePath, backupPath);
        err(`Corrupted file backed up to ${backupPath}`);
      } catch (_) {}
    }
    return fallback;
  }
...
function atomicWriteJson(filePath, data) {
  try {
    ...
    err(`atomicWriteJson error writing to ${filePath}: ${e.message}`);
  }
}

function loadUsers() {
  return normalizeUsersObject(readJsonFile(DATA_FILE, {}));
}

function saveUsers(users) {
  try {
    const disk = normalizeUsersObject(readJsonFile(DATA_FILE, {}));
    const merged = { ...disk, ...(users && typeof users === 'object' ? users : {}) };
    atomicWriteJson(DATA_FILE, merged);
...
function reloadUsersFromDisk() {
  try {
    ...
    const fresh = readJsonFile(DATA_FILE, null);
